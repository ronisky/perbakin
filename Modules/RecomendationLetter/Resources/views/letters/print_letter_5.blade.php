...
{
    $this->Cell(10);
    $this->MultiCell(160,5,"Selaku Ketua rombongan,  dengan ini mengajukan ".ucwords($letter[0]->letter_category_name)." pada ".DateFormatHelper::dateIn($letter[0]->waktu_mulai)." s.d. ".DateFormatHelper::dateIn($letter[0]->waktu_selesai)." untuk Latihan rutin dan dalam rangka ".ucwords($letter[0]->dalam_event)." yang akan dilaksanakan di:",0,'J',0);

    $this->Cell(15);
    $this->Cell(2, 5, '1.', 0, 0, 'C');
    $this->Cell(32, 5, ucwords($letter[0]->lokasi1), 0, 0, 'L');

    $this->Ln(5);
    $this->Cell(15);
    $this->Cell(2, 5, '2.', 0, 0, 'C');
    $this->Cell(32, 5, ucwords($letter[0]->lokasi2), 0, 0, 'L');

    $this->Ln(5);
    $this->Cell(15);
    $this->Cell(2, 5, '3.', 0, 0, 'C');
    $this->Cell(32, 5, ucwords($letter[0]->lokasi3), 0, 0, 'L');

    $this->Ln(5);
    $this->Cell(15);
    $this->Cell(2, 5, '4.', 0, 0, 'C');
    $this->Cell(32, 5, ucwords($letter[0]->lokasi4), 0, 0, 'L');
}

function BodyDua($letter)
{
    $this->Cell(10);
    $this->Cell(15, 5, '2. Adapun anggota rombongan kami berjumlah  ( '.$letter[0]->jumlah_anggota.' ) orang terlampir', 0, 0, 'L');
